...
def transform():
    ...
    if os.path.exists('/opt/airflow/data/output'):
        shutil.rmtree("/opt/airflow/data/output")
        os.makedirs("/opt/airflow/data/output", exist_ok=True)

    #Create Spark session
    spark = SparkSession.builder.appName("Data_Transformation").getOrCreate()

    #Read cleaned data from /opt/airflow/data/processed
    ...
    print('=====================================================')
    print('writing transformed dataframes to /opt/airflow/data/output now...')
    print('=====================================================')
    
    #Write transformed data to /opt/airflow/data/output in parquet format
    ...
    dim_geolocation_df.write.mode("overwrite").parquet("/opt/airflow/data/output/dim_geolocation")
    dim_last_order_date_df.write.mode("overwrite").parquet("/opt/airflow/data/output/dim_last_order_date")
